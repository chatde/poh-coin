...
const socialLinks = [
  ...
  {
    label: "Instagram",
    href: "https://instagram.com/project_poh",
    icon: (
      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
...
export function Footer() {
  ...
  return (
    <footer className="relative overflow-hidden bg-gradient-to-b from-background to-surface">
      {/* Nebula glow at top */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -top-20 left-1/2 -translate-x-1/2"
      >
        <div className="h-40 w-[600px] rounded-full bg-accent/8 blur-[80px]" />
      </div>
      ...
      >
        <div className="flex items-center gap-1 rotate-[-30deg]">
          <div className="h-px w-16 bg-gradient-to-r from-transparent to-voyager-gold/40" />
          <div className="h-1.5 w-1.5 rounded-full bg-voyager-gold/60" />
        </div>
      </div>
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col items-center gap-8 md:flex-row md:justify-between">
          ...
          <div className="text-center md:text-left">
            <Link href="/" className="text-xl font-bold tracking-tight text-foreground">
              Project&nbsp;<span className="text-accent-light">POH</span>
            </Link>
            <p className="mt-2 text-sm text-voyager-gold">
              Changing the trajectory of humankind
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            {socialLinks.map((social) => (
              <a
                ...
                className="text-foreground/50 transition-colors hover:text-accent-light"
              >
                {social.icon}
              </a>
            ))}
        ...
        {/* Gradient divider */}
        <div className="my-8 h-px bg-gradient-to-r from-transparent via-surface-light to-transparent" />
